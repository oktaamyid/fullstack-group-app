...
async function createSplitBill(req, res) {
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            // Calculate user portion for transaction
            const userPortion = memberData.filter(m => m.isUser).reduce((acc, m) => acc + m.amount, 0);

            // Create transaction record if syncToPersonal and user portion > 0
            let transactionId = null;
            if (syncToPersonal && userPortion > 0) {
                ...
                await tx.wallet.update({
                    where: { id: walletId },
                    data: { balance: { decrement: userPortion } },
                });
            }
            ...
            for (const m of memberData) {
                const cm = await tx.splitBillMember.create({
                    data: {
                        ...
                        status: m.isUser ? 'PAID' : 'UNPAID',
                        paidAt: m.isUser ? new Date() : null,
                    },
                });
...
async function updateSplitBill(req, res) {
    ...
    try {
        ...
        const updated = await prisma.$transaction(async (tx) => {
            const existingMembers = await tx.splitBillMember.findMany({ where: { splitBillId } });
            if (memberData) {
                        ...
                        ? {
                            members: {
                                create: memberData.map((member) => {
                                    const prev = existingMembers.find(em => em.friendName === member.friendName);
                                    return {
                                        friendName: member.friendName,
                                        amount: member.amount,
                                        isUser: member.isUser || false,
                                        status: prev ? prev.status : (member.isUser ? 'PAID' : 'UNPAID'),
                                        paidAt: prev ? prev.paidAt : (member.isUser ? new Date() : null),
                                        paymentTxId: prev ? prev.paymentTxId : null,
                                    };
                                }),
                            },
                        }
...
async function deleteSplitBill(req, res) {
    try {
        ...
        if (!splitBill || splitBill.userId !== req.user.id) {
            return sendError(res, 'Split bill not found', 404);
        }

        await prisma.$transaction(async (tx) => {
            if (splitBill.syncToPersonal && splitBill.transactionId) {
                const txRecord = await tx.transaction.findUnique({
                    where: { id: splitBill.transactionId },
                });

                if (txRecord && txRecord.walletId) {
                    await tx.wallet.update({
                ...
                await tx.transaction.delete({ where: { id: splitBill.transactionId } });
            }

            await tx.splitBill.delete({ where: { id: Number(id) } });
        });

        return sendSuccess(res, null, 'Split bill deleted successfully');
    } catch (error) {
...
async function getSplitBillByIdPublic(req, res) {
    try {
        const { id: hash } = req.params;
        
        let splitBillId = Number(hash);
        if (Number.isNaN(splitBillId) || !Number.isInteger(splitBillId)) {
            const num = parseInt(hash, 36);
            splitBillId = num / 792384;
        }
        ...
        const splitBill = await prisma.splitBill.findUnique({
            where: { id: splitBillId },
            include: {
                members: { include: { itemAssignments: true } },
        ...
        return sendSuccess(res, { splitBill });
    } catch (error) {
        return sendError(res, 'Failed to fetch split bill', 500, { error: error.message });
    }
}
