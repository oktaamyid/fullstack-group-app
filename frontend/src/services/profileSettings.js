...
function normalizeCategoryList(list = [], fallback = []) {
  ...
  return [
    ...new Set(
      merged.filter(Boolean).map((item) => String(item).trim().toUpperCase()),
    ),
  ];
}
  ...
  return {
    ...
    categories: {
      EXPENSE: normalizeCategoryList(
        categories.EXPENSE,
        DEFAULT_TRANSACTION_CATEGORIES.EXPENSE,
      ),
      INCOME: normalizeCategoryList(
        categories.INCOME,
        DEFAULT_TRANSACTION_CATEGORIES.INCOME,
      ),
    },
...
async function authRequest(path, options = {}) {
  ...
  if (!response.ok || !payload?.success) {
    throw new Error(
      payload?.data?.detail || payload?.message || "Profile request failed",
    );
  }
...
export async function fetchCustomCategories() {
  try {
    const data = await authRequest("/categories");
    return data.categories || { EXPENSE: [], INCOME: [], categoryIcons: {} };
  } catch {
    // Fallback to empty if fetch fails
    return { EXPENSE: [], INCOME: [], categoryIcons: {} };
  }
}
...
export async function saveCustomCategories(categories) {
  try {
    ...
    // Log error but don't throw - allow app to continue with localStorage fallback
    console.warn("Failed to save categories to backend:", error.message);
    throw error;
  }
}
...
export async function addCategorySetting(userId, type, label, icon = "") {
  ...
  try {
    await saveCustomCategories({
      EXPENSE: next.categories.EXPENSE,
      INCOME: next.categories.INCOME,
      categoryIcons: next.categoryIcons,
    });
  } catch (error) {
    console.warn(
      "Category added locally but failed to sync with backend:",
      error.message,
    );
  }
...
export async function removeCategorySetting(userId, type, categoryKey) {
  const normalizedType = type === "INCOME" ? "INCOME" : "EXPENSE";
  ...
  const current = getLocalSettings(userId);
  const nextList = (current.categories[normalizedType] || []).filter(
    (item) => item !== key,
  );
  ...
  try {
    ...
    console.warn(
      "Category removed locally but failed to sync with backend:",
      error.message,
    );
  }
