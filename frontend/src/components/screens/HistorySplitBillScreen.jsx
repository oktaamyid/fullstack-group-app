...
export function HistorySplitBillScreen({ mainLogo }) {
  ...
  const refreshSplitBills = useCallback(async () => {
    ...
    try {
      const [splitBillData, analyticsData] = await Promise.all([
        getSplitBills(),
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-4">
            <article className="rounded-xl lg:rounded-2xl border border-[#1c1c13] bg-[#f8f4e4] p-4 lg:p-6 shadow-[2px_2px_0_#1c1c13]">
              ...
              <p className="text-lg lg:text-3xl font-black text-[#6366f1] mt-2">
            ...
            <article className="hidden lg:block rounded-2xl border border-[#1c1c13] bg-[#fff9dc] p-6 shadow-[2px_2px_0_#1c1c13]">
              ...
              <p className="text-3xl font-black text-[#4648d4] mt-2">
                {toRupiah(summary.paid)}
              </p>
            </article>
          </div>
          ...
          <section className="rounded-xl lg:rounded-2xl border border-[#1c1c13] bg-white p-4 lg:p-6 shadow-[2px_2px_0_#1c1c13] space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <article className="rounded-lg border border-[#1c1c13] bg-[#f8f4e4] p-3">
                ...
                <p className="mt-1 text-sm lg:text-base font-black text-[#1c1c13]">
                  {toCurrency(analytics?.totals?.weeklyTotal || 0)}
                </p>
              </article>
              <article className="rounded-lg border border-[#1c1c13] bg-[#f8f4e4] p-3">
                <p className="text-[10px] font-bold uppercase text-[#464554]">
                  {t("topCategoryLabel", "Top Category")}
                </p>
                <p className="mt-1 text-sm lg:text-base font-black text-[#1c1c13]">
                  {analytics?.topCategory?.name || t("noData", "No Data")}
                </p>
              </article>
            </div>
                    ...
                    >
                      <div className="w-full rounded-sm border border-[#1c1c13] bg-[#e1e0ff]">
                        <div
                          ...
                          style={{
                            height: `${Math.max(8, Math.round((point.amount / maxTrendAmount) * 60))}px`,
                          }}
                        />
                      </div>
                      <span className="text-[9px] font-black uppercase text-[#464554]">
                        {point.day}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="col-span-6 text-xs font-semibold">
                    {t("noWeeklyTrendData", "Belum ada data tren mingguan.")}
                  </p>
                )}
              </div>
            </div>
            ...
            <section className="rounded-xl lg:rounded-2xl border border-[#1c1c13] bg-white p-4 lg:p-6 shadow-[2px_2px_0_#1c1c13]">
              <h2 className="text-sm font-black uppercase">
                {editingId
                  ...
                  : t("newSplitBill", "Buat Split Bill Baru")}
              </h2>
              <form className="mt-3 space-y-3" onSubmit={onSubmit}>
                <label className="block text-[11px] font-bold uppercase">
                  ...
                  <input
                    ...
                    onChange={onChange}
                    placeholder={tr(
                      "Example: Dinner with classmates",
                      "Contoh: Makan bareng kelas",
                    )}
                ...
                <label className="block text-[11px] font-bold uppercase">
                  ...
                  <input
                    ...
                    onChange={onChange}
                    placeholder={tr(
                      "Dinner after midterms",
                      "Makan malam habis UTS",
                    )}
                    className="mt-1 min-h-11 w-full rounded-xl lg:rounded-2xl border border-[#1c1c13] bg-[#fffbeb] px-3 text-sm"
                  />
                </label>
                ...
                <p className="text-[11px] font-semibold text-[#464554]">
                  {tr(
                    "Amount will be split equally across friends.",
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    {isSubmitting
                      ? t("saving", "Menyimpan...")
                      : editingId
                        ? t("updateSplit", "Update Split")
                        : t("saveSplit", "Simpan Split")}
                  </button>
                  <button
                    type="button"
                    onClick={resetForm}
              ...
              <div className="rounded-lg lg:rounded-xl border border-[#1c1c13] bg-white p-4 lg:p-6 shadow-[2px_2px_0_#1c1c13]">
                ...
                {!isLoading && splitBills.length === 0 ? (
                  <p className="rounded-lg lg:rounded-xl border border-[#1c1c13] bg-[#fffbeb] px-4 py-3 text-sm font-semibold">
                    {tr(
                      "No split bills yet. Create one from Split Bill tab.",
                ...
                {splitBills.map((splitBill) => (
                  <article
                    key={splitBill.id}
                    className="rounded-lg lg:rounded-xl border border-[#1c1c13] bg-[#fffbeb] p-4 lg:p-5 shadow-[2px_2px_0_#1c1c13] mb-3 lg:mb-4"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs font-semibold">
                          Total: {toRupiah(splitBill.totalAmount)}
                        </p>
                      </div>
                      <span
                    ...
                    <ul className="mt-3 space-y-2 border-t border-[#1c1c13] pt-3">
                      {splitBill.members.map((member) => (
                        <li
                          key={member.id}
                          className="rounded-lg border border-[#1c1c13] bg-white p-2 lg:p-3"
                        >
        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          <section className="rounded-lg lg:rounded-2xl border border-[#1c1c13] bg-[#6366f1] text-white p-4 lg:p-6 shadow-[2px_2px_0_#1c1c13]">
            ...
            <p className="text-xs lg:text-sm opacity-90 mt-2 font-medium">
              {t("manageSharedExpenses", "Manage shared expenses")}
            </p>
          </section>
          ...
          <div className="space-y-3">
            {splitBills.length === 0 ? (
              <div className="rounded-lg lg:rounded-xl border border-[#1c1c13] bg-[#f8f4e4] p-4 lg:p-6 shadow-[2px_2px_0_#1c1c13] text-center">
                <p className="text-xs lg:text-sm font-bold text-[#464554]">
                  {t("noActiveSplitBills", "No active split bills yet")}
                </p>
                <p className="text-[11px] text-[#464554] mt-1">
                  {t(
                    "createFirstSplit",
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="text-sm lg:text-base font-black text-[#1c1c13] truncate">
                              {member.friendName}
                            </h4>
                            <p
                              className={`text-xs ${statusColor} font-bold uppercase`}
                            >
                              {statusLabel}
                            </p>
                          </div>
                        ...
                        <div className="text-right">
                          <div
                            className={`font-black text-base lg:text-lg ${amountColor}`}
                          >
                            {amountDisplay}
                          </div>
                        </div>
                      ...
                      <div className="border-t border-[#1c1c13] pt-3 flex justify-between items-center">
                        <span className="text-[10px] lg:text-xs font-medium text-[#464554]">
                          {splitBill.title}
                        </span>
                        {actionButton && (
                          <button
                            ...
                            className={`text-xs font-black px-2 lg:px-3 py-1 rounded border border-[#1c1c13] shadow-[1px_1px_0_#1c1c13] transition-all ${"bg-[#fbbf24] text-[#1c1c13] hover:-translate-y-0.5"}`}
                          >
                            {actionButton}
                          </button>
                        )}
