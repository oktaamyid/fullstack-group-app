...
const defaultProfileForm = { name: '', email: '' }
...
export function ProfileSettingsScreen({ mainLogo }) {
  ...
  const onToggleSetting = useCallback(
    (field) => {
      setSettings((prev) => {
        ...
        if (authUser?.id) saveLocalSettings(authUser.id, next)
        return next
      })
    },
    [authUser?.id]
  ...
  const handleExportCSV = useCallback(async () => {
    try {
      ...
      a.click();
      a.remove();
      setMessage(tr("Export successful", "Ekspor berhasil"));
    } catch (error) {
      console.error(error);
      setErrorMessage(tr("Failed to export CSV", "Gagal mengekspor CSV"));
    }
  }, [tr]);
  ...
  return (
    ...
    >
      <div className="flex rounded-xl border-2 border-[#1c1c13] bg-[#fffbeb] p-1 w-full max-w-md mx-auto mb-6 shadow-[2px_2px_0_#1c1c13]">
        <button
          className={`flex-1 rounded-lg py-2 text-[10px] font-black uppercase tracking-wide transition-colors ${activeTab === "APLIKASI" ? "bg-[#4648d4] text-white shadow-sm" : "text-[#1c1c13] hover:bg-[#ffc329]"}`}
          onClick={() => setActiveTab("APLIKASI")}
        >
          {tr("Application", "Aplikasi")}
        </button>
        <button
          className={`flex-1 rounded-lg py-2 text-[10px] font-black uppercase tracking-wide transition-colors ${activeTab === "DATA" ? "bg-[#4648d4] text-white shadow-sm" : "text-[#1c1c13] hover:bg-[#ffc329]"}`}
          onClick={() => setActiveTab("DATA")}
        >
          {tr("Data", "Data")}
        </button>
        <button
          className={`flex-1 rounded-lg py-2 text-[10px] font-black uppercase tracking-wide transition-colors ${activeTab === "BANTUAN" ? "bg-[#4648d4] text-white shadow-sm" : "text-[#1c1c13] hover:bg-[#ffc329]"}`}
      ...
      <div className="max-w-4xl mx-auto space-y-6 pb-6">
        ...
        {errorMessage ? (
          <Alert type="error" onClose={() => setErrorMessage('')}>{errorMessage}</Alert>
        ) : null}

        {activeTab === 'APLIKASI' && (
          <div className="space-y-6 lg:grid lg:grid-cols-2 lg:gap-6 lg:space-y-0">
            {/* Account Info */}
            <section className="rounded-xl border-2 border-[#1c1c13] bg-[#fffbeb] p-6 shadow-[4px_4px_0_#1c1c13] lg:col-span-2 flex justify-between items-center">
              <div>
                <p className="text-[11px] font-black uppercase text-[#1c1c13]">{tr('Your Account', 'Akun Anda')}</p>
              ...
              <div className="h-16 w-16 bg-[#4648d4] rounded-full border-2 border-[#1c1c13] shadow-[2px_2px_0_#1c1c13] flex items-center justify-center">
                <span className="text-2xl font-black text-white">{authUser?.name?.charAt(0) || 'U'}</span>
              </div>
            </section>

            {/* Edit Profile */}
            <section className="rounded-xl border-2 border-[#1c1c13] bg-white p-6 shadow-[4px_4px_0_#1c1c13]">
              <h3 className="mb-4 text-sm font-black uppercase text-[#1c1c13]">{tr('Edit Profile', 'Ubah Profil')}</h3>
              {isLoading ? (
                ...
                <form className="space-y-4" onSubmit={onSaveProfile}>
                  ...
                  <Button type="submit" disabled={isProfileSubmitting} fullWidth>
            ...
            {/* Security */}
            <section className="rounded-xl border-2 border-[#1c1c13] bg-white p-6 shadow-[4px_4px_0_#1c1c13]">
              <h3 className="mb-4 text-sm font-black uppercase text-[#1c1c13]">{tr('Security', 'Keamanan')}</h3>
              <form className="space-y-4" onSubmit={onSavePassword}>
                <label className="block text-[11px] font-black uppercase text-[#1c1c13]">
                  {tr('Current Password', 'Password Saat Ini')}
                  <input
                    type="password"
                ...
                <label className="block text-[11px] font-black uppercase text-[#1c1c13]">
                  {tr('Confirm New Password', 'Konfirmasi Password Baru')}
                  <input
                    type="password"
                ...
                <Button type="submit" variant="accent" disabled={isPasswordSubmitting} fullWidth>
            ...
            <section className="rounded-xl border-2 border-[#1c1c13] bg-white p-6 shadow-[4px_4px_0_#1c1c13] lg:col-span-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <label className="block text-[11px] font-black uppercase text-[#1c1c13]">
                  {tr('Daily Limit Mode', 'Mode Batas Harian')}
                  <select
                    value={settings.dailyLimitMode || 'AUTO'}
                    onChange={(event) => onConfigChange('dailyLimitMode', event.target.value)}
                    className="mt-1.5 min-h-11 w-full rounded-lg border-2 border-[#1c1c13] bg-white px-4 text-sm font-bold shadow-[2px_2px_0_#1c1c13] outline-none focus:border-[#6366f1] cursor-pointer"
                  >
                    <option value="AUTO">{tr('Automatic', 'Otomatis')}</option>
                    <option value="MANUAL">{tr('Manual Set', 'Atur Manual')}</option>
                  </select>
                </label>

                {settings.dailyLimitMode === 'MANUAL' && (
                  <label className="block text-[11px] font-black uppercase text-[#1c1c13]">
                    {tr('Manual Daily Limit Amount', 'Nominal Batas Harian Manual')}
                    <input
                      type="number"
                      min="1"
                      value={settings.manualDailyLimit || 100000}
                      onChange={(event) => onConfigChange('manualDailyLimit', Number(event.target.value))}
                      className="mt-1.5 min-h-11 w-full rounded-lg border-2 border-[#1c1c13] bg-white px-4 text-sm font-bold shadow-[2px_2px_0_#1c1c13] outline-none focus:border-[#6366f1]"
                    />
                  </label>
                )}
        ...
        {activeTab === 'DATA' && (
          <div className="space-y-6">
            ...
            <section className="rounded-xl border-2 border-[#1c1c13] bg-white p-6 shadow-[4px_4px_0_#1c1c13] flex items-center justify-between">
              <div>
                <h3 className="text-sm font-black uppercase text-[#1c1c13]">{tr('Export Data', 'Ekspor Data')}</h3>
                <p className="text-xs text-gray-500 mt-1">{tr('Download all your transaction data as CSV.', 'Unduh semua data transaksi Anda dalam format CSV.')}</p>
              </div>
              <Button variant="outline" onClick={handleExportCSV}>
                {tr('Export CSV', 'Ekspor CSV')}
              </Button>
            </section>
          </div>
        )}
        ...
        {activeTab === 'BANTUAN' && (
          <div className="space-y-6">
            ...
            <section className="rounded-xl border-2 border-[#1c1c13] bg-white p-6 shadow-[4px_4px_0_#1c1c13]">
               <h3 className="mb-2 text-sm font-black uppercase text-[#1c1c13]">{tr('About LIVO', 'Tentang LIVO')}</h3>
               <p className="text-sm text-gray-600 mb-6">{tr('Version 1.0.0 (Beta)', 'Versi 1.0.0 (Beta)')}</p>
               
               <Button
                  variant="danger"
                  fullWidth
                  className="rounded-xl text-base shadow-[4px_4px_0_#1c1c13] flex justify-center items-center gap-2"
